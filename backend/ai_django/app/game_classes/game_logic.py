...
class Logic:
    @staticmethod
    def calculate_legal_moves(player, other_player, board, move_history):
        in_check = Logic.in_check(board, player, other_player)
        ...
        def helper(piece):
            ...
            if in_check:
                ...
                Logic.validate_moves(board, piece, player, other_player)
    ...
    @staticmethod
    def in_check_after_move(board, move_history, player, other_player, from_loc, to_loc) -> bool:
    ...
    @staticmethod
    def validate_moves(board, piece, player, other_player):
        if piece.get_type() == KING:
            player.pieces[KING].legal_moves = list(filter(
                lambda move: not Logic.in_check_after_move(
                    board, player, other_player, piece.loc, move
                ),
    ...
    @staticmethod
    def in_check(board, player, other_player) -> bool:
        ...
        for piece_type in [KNIGHTS, BISHOPS, ROOKS, QUEENS]:
            for piece in other_player.pieces[piece_type]:
        ...
        if player.pieces[KING].loc in other_player.pieces[KING].legal_moves:
            return True
